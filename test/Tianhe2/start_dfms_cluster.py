...
def main():
    ...
    parser.add_option("-R", "--spead2_receiver_sender", action="store", type="string",
                      dest="spead2_receiver_sender", help="spead2 receiver or sender", default="sender")
    ...
    if (options.num_islands == 1):
        if (rank != 0):
            if (run_proxy and rank == 1):
                ...
                spead_conf = json.loads(open('/BIGDATA/ac_shao_tan_1/OSKAR/IDOS/spead/sender/spead_send.json').read())
                spead_streams=spead_conf['streams']
                spead_streams.append({'port': 41000, 'host': origin_ip})
                with open('/BIGDATA/ac_shao_tan_1/OSKAR/IDOS/spead/sender/spead_send.json','w') as f:
